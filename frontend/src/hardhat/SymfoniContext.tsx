...
import ProjectDeployment from "./deployments/localhost/Project.json";
import { Project } from "./typechain/Project";
import { Project__factory } from "./typechain/factories/Project__factory";
...
export const ProjectContext = React.createContext<SymfoniProject>(emptyContract);
...
export interface SymfoniProject {
    instance?: Project;
    factory?: Project__factory;
}
...
export const Symfoni: React.FC<SymfoniProps> = ({
    ...
    const [Project, setProject] = useState<SymfoniProject>(emptyContract);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const doAsync = async () => {
            ...
            const finishWithContracts = (text: string) => {
                setProject(getProject(_provider, _signer))
                finish(text)
            }
    ...
    const getProject = (_provider: providers.Provider, _signer?: Signer) => {

        const contractAddress = ProjectDeployment.receipt.contractAddress
        const instance = _signer ? Project__factory.connect(contractAddress, _signer) : Project__factory.connect(contractAddress, _provider)
        const contract: SymfoniProject = {
            instance: instance,
            factory: _signer ? new Project__factory(_signer) : undefined,
        }
        return contract
    }
        ;
